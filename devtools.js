...
chrome.devtools.panels.create("BlueKai Debugger",
    ...
    function(extensionPanel) {
        ...
        alert('chrome.devtools.panels.create : callback');
        extensionPanel.onShown.addListener(function(extPanelWindow) {                            

            _window = extPanelWindow;
            extPanelWindow.document.body.append('hello again');
...
function BlueKaiChromeDebugger() {};

BlueKaiChromeDebugger.handleBlueKaiHeaders = function(har_entry) {
...
BlueKaiChromeDebugger._processLogMessage = function(message, context) {
    var meta = message[0];
    if (!meta) {
...
BlueKaiChromeDebugger._buildMessageObjects = function(header_map) {
    const normal_header_prefix = "X-Wf-1-1-1-";

    return this._collectMessageObjectsForPrefix(header_map, normal_header_prefix);
};

BlueKaiChromeDebugger._collectMessageObjectsForPrefix = function(header_map, prefix) {
    
    debugger; // roshan : customised
    var results = [];
...
BlueKaiChromeDebugger._checkProtoVersion = function(proto_header) {
    if (!proto_header) {
...
chrome.devtools.network.addRequestHeaders({
    "BlueKai Chrome Debugger": "0.0.6"
});

chrome.devtools.network.getHAR(function(result) {
    var entries = result.entries;
    if (!entries.length) {
        Console.warn("BlueKai Chrome Debugger suggests that you reload the page to track" +
            " BlueKai Chrome Debugger messages for all the requests");
    }
    for (var i = 0; i < entries.length; ++i)
        BlueKaiChromeDebugger.handleFirePhp_headers(entries[i]);

    chrome.devtools.network.onRequestFinished.addListener(
        BlueKaiChromeDebugger.handleBlueKaiHeaders.bind(BlueKaiChromeDebugger));
});
//*/